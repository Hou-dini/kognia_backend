...
async def run_agent_task(job_id: str, prompt: str, user_id: uuid.UUID, session_id: str):
    ...
    if not runner:
        ...
        await update_job_status(job_id, "failed") # <<< CHANGED: "error" to "failed"
        return
    
    try:
        await update_job_status(job_id, "processing") # <<< CHANGED: "running" to "processing"
        ...
        try:
            ...
            # This might happen if ADK session already exists, which is fine, but log the specific error
            print(f"[Job {job_id}]: ADK Session creation warning (might already exist or schema issue): {e}")
        ...
        async with db_pool.acquire() as conn:
            ...
            await update_job_status(job_id, "completed") # <<< CHANGED: "complete" to "completed"
    ...
    try:
        async with db_pool.acquire() as conn:
            ...
            new_job_id = await conn.fetchval(
                ...
                authenticated_user_id, uuid.UUID(client_session_id), job_request.prompt, 'pending' # 'pending' is still valid
            )
...
async def get_job_status(job_id: str, current_user_id: uuid.UUID = Depends(get_current_user_id)):
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            ...
            status_val = job_record['status'] # Renamed 'status' to 'status_val' to avoid shadowing

            if status_val == "completed": # <<< CHANGED: "complete" to "completed"
                report_content = await conn.fetchval(
                    "SELECT content FROM reports WHERE job_id = $1 AND user_id = $2",
                    job_uuid, current_user_id
                )
                return ReportResponse(job_id=job_id, status="completed", report=report_content) # <<< CHANGED: "complete" to "completed"
