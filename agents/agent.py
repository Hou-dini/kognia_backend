...
2.  **Intelligent Chaining:**
    ...
    *   **Multi-Stage Requests:** Process complex requests by chaining agents logically. For example:
        ...
        *   "Write a strategic report on [Topic], identifying several key [Entities] or [Actors] behind this [Topic]" -> "`MarketIntel Analyst`" -> "`StrategicSwot Evaluator`" -> "`StrategicReport Architect`".
